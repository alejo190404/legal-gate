...
curl -fsS \
  ...
  -d '{"intakeEmail":"intake@familia.test","routingRules":[{"name":"Familia intake","urgentKeywords":["audiencia","captura"],"consultationWindows":["LUN-VIE 09:00-13:00"],"destinationEmail":"notificaciones@familia.test"}]}' \
  "http://localhost:${PORT}/api/tenants/familia-legal/settings" \
  | jq -e '.tenantId == "familia-legal" and .routingRules[0].urgentKeywords[0] == "audiencia" and .intakeEmail == "intake@familia.test"' >/dev/null

curl -fsS "http://localhost:${PORT}/api/tenants/familia-legal/settings" \
  | jq -e '.tenantId == "familia-legal" and .intakeEmail == "intake@familia.test" and (.routingRules | length) == 1' >/dev/null
