...
**The Consultation Thread is anchored flat, not chained.** Every message LegalGate sends about a Consultation points at the potential client's original email rather than at the previous message in the exchange. Mail clients group on a shared root, so a flat anchor and a true chain land in the same conversation; the chain would have cost persisted message identifiers on two more tables to buy correct nesting in a handful of readers.
