...
class ProxyIntegrationTests {
    ...
    static void startBackend() throws IOException {
        ...
        backend.createContext("/cases/42", ProxyIntegrationTests::respondWithCase);
    ...
    static void gatewayProperties(DynamicPropertyRegistry registry) {
        registry.add("legalgate.gateway.api-key", () -> "test-gateway-key");
    ...
    @Test
    void authenticatedRequestsAreProxiedToConfiguredBackend() throws Exception {
        mockMvc.perform(get("/api/backend/cases/42?include=summary")
                        .header("X-Gateway-Api-Key", "test-gateway-key"))
                .andExpect(status().isOk())
                .andExpect(header().string("X-Proxied-By", "legal-gate-gateway"))
                .andExpect(jsonPath("$.id").value("42"))
                .andExpect(jsonPath("$.path").value("/cases/42"))
    ...
    void unsupportedOrUnavailableBackendResponsesUseGatewayFallbackShape() throws Exception {
        mockMvc.perform(post("/api/backend/cases/42")
                        .header("X-Gateway-Api-Key", "test-gateway-key")
